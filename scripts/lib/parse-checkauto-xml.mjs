...
function riskFromCode(code, { emptyIsOk = false } = {}) {
  if (!Number.isFinite(code))
    return { level: "unavailable", label: "Indisponível" };
  ...
  return {
    level: "unavailable",
    label: `Retorno não reconhecido (código ${code})`,
  };
}
...
function parseRegistroEstadual(xml) {
  const block = blockOf(innerOf(xml, "Veiculo"), "RegistroEstadual");
  const code = resumoCode(xml, "RegistroEstadual");
  const risk = riskFromCode(block ? code : null);
  ...
  if (/roub|furto|baixa|leil/i.test(restricoes.join(" "))) {
    ...
    debitoIpva &&
    /(?:^|[.;]\s*)existe\s+d[eé]bito/i.test(debitoIpva)
  ) {
    level = "attention";
    label = "Com débitos / restrições leves";
  } else if (code === 1 && situacao) {
    level = restricoes.length ? "attention" : "ok";
    label = restricoes.length
      ? "Veículo encontrado — ver restrições"
      : "Veículo encontrado";
  }
  ...
  return {
    ...
    fields: fields([
      ...
      [
        "Ano fab / mod",
        `${textOf(block, "AnoFabricacao") || "—"} / ${textOf(block, "AnoModelo") || "—"}`,
      ],
      ...
      [
        "Potência",
        textOf(block, "PotenciaVeiculo")
          ? `${textOf(block, "PotenciaVeiculo")} cv`
          : "",
      ],
      ...
      [
        "Valor IPVA",
        textOf(block, "ValorDebitoIPVA")
          ? moneyBr(textOf(block, "ValorDebitoIPVA"))
          : "",
      ],
      [
        "Valor licenciamento",
        textOf(block, "ValorDebitoLicenciamento")
          ? moneyBr(textOf(block, "ValorDebitoLicenciamento"))
          : "",
      ],
...
function parseRouboFurto(xml) {
  const block = blockOf(innerOf(xml, "Veiculo"), "RouboFurtoFederal");
  ...
  let label = "Retorno não confirmado";
  if (!block || code == null) {
    level = "unavailable";
    label = "Indisponível";
    ...
    records.some((record) => {
      const category =
        record.find((entry) => entry.label === "Categoria")?.value || "";
      return (
        /roubado|furtado/i.test(category) &&
        !/recuperado|baixado/i.test(category)
      );
    })
  ) {
    level = "alert";
    label = "Ocorrência crítica";
  } else if (regs.length > 0 || code === 1) {
    level = "attention";
    label = regs.length
      ? `${regs.length} ocorrência(ões) — ver histórico`
      : "Informação encontrada — detalhes não informados";
  }
  ...
  return {
    ...
    fields: fields([
      [
        "Qtde ocorrências",
        textOf(block, "QtdeOcorrencias") || String(regs.length),
      ],
      ["Retorno", textOf(block, "DescricaoRetorno")],
    ]),
    records,
...
function parseAgregado(xml) {
  const block = blockOf(innerOf(xml, "Veiculo"), "AgregadoChassiMotor");
  const code = resumoCode(xml, "AgregadoChassiMotor");
  const risk = riskFromCode(block ? code : null);
  return {
    key: "agregado",
    ...DOSSIER_COPY.agregado,
    riskLevel: block && code === 1 ? "ok" : risk.level,
    riskLabel:
      block && code === 1 ? "Dados de chassi / motor retornados" : risk.label,
    retorno: textOf(block, "DescricaoRetorno"),
    fields: fields([
      ["Chassi", maskChassi(textOf(block, "Chassi"))],
      ["Placa", maskPlate(textOf(block, "Placa"))],
...
function parseDecodificador(xml) {
  const block = blockOf(innerOf(xml, "Veiculo"), "DecodificadorChassi");
  ...
  return {
    ...
    riskLevel: block && code === 1 ? "ok" : risk.level,
    riskLabel: block && code === 1 ? "VIN decodificado" : risk.label,
    retorno: textOf(block, "DescricaoRetorno"),
    fields: fieldsList,
...
function parsePrecificador(xml) {
  const block = blockOf(innerOf(xml, "Veiculo"), "Precificador");
  const tabela = blockOf(block, "TabelaFipe") || block;
  const rawCode = textOf(tabela, "CodigoRetorno");
  const code = rawCode
    ? /^\d+$/.test(rawCode)
      ? Number(rawCode)
      : null
    : resumoCode(xml, "Precificador");
  ...
  return {
    ...
    riskLevel: records.length ? "ok" : riskFromCode(block ? code : null).level,
    riskLabel: records.length
      ? `${records.length} referência(s) FIPE`
      : riskFromCode(block ? code : null).label,
    retorno: textOf(tabela, "DescricaoRetorno"),
    fields: fields([
      [
        "Qtd registros",
        textOf(tabela, "QtdRegistros") || String(records.length),
      ],
      ["Retorno", textOf(tabela, "DescricaoRetorno")],
    ]),
    records,
...
function parseLeilao(xml) {
  const block = blockOf(innerOf(xml, "Veiculo"), "LeilaoRemarketing");
  ...
  const records = regs.map((reg) =>
    fields([
      [
        "Descrição",
        textOf(reg, "Descricao") || textOf(reg, "DescricaoRetorno"),
      ],
  ...
  const noRecord =
    ...
    records.length === 0 &&
    (code === 2 ||
      /^(?:n[aã]o\s+h[aá]|sem)\s+registro/i.test(
        textOf(block, "DescricaoRetorno"),
      ));
  return {
    key: "leilao",
    ...DOSSIER_COPY.leilao,
    riskLevel: noRecord
      ...
      : records.length
        ? "alert"
        : riskFromCode(block ? code : null, { emptyIsOk: true }).level,
    riskLabel: noRecord
      ? "Sem registro de leilão/sinistro"
      : records.length
        ? "Registro de leilão/sinistro"
        : riskFromCode(block ? code : null, { emptyIsOk: true }).label,
    retorno: textOf(block, "DescricaoRetorno"),
...
export function parseCheckAutoDossier(xmlPathOrString) {
  ...
  if (!xml.trim() || !/<Veiculo\b/i.test(xml)) {
    return {
      ok: false,
  ...
  const marcaModelo =
    reg.fields.find((f) => f.label === "Marca / modelo")?.value || "";
  ...
  const history = [
    {
      ...
      label: "Leilão",
      status:
        sections[5].riskLevel === "unavailable"
          ? null
          : sections[5].riskLevel === "ok"
            ? "Sem Registro"
            : sections[5].riskLabel,
    },
    {
      key: "sinistro",
      ...
      status:
        sections[5].riskLevel === "ok" &&
        /sinistro|perda/i.test(sections[5].retorno)
          ? "Sem Registro"
          : null,
    },
    {
    ...
    },
    {
      ...
      status:
        sections[0].riskLevel === "unavailable" ? null : sections[0].riskLabel,
    },
  ];

  const available = sections.some((s) => s.riskLevel !== "unavailable");
  // A successful lookup is not a clean result; incomplete or advisory returns remain visible.
  const strictClear =
    history.every((item) => item.status === "Sem Registro") &&
    sections.every((section) => section.riskLevel === "ok");
  ...
  return {
    ...
    vehicleHint: {
      ...
      combustivel:
        reg.fields.find((f) => f.label === "Combustível")?.value || "",
