...
export function parseSourceHashFromDistSubject(subject = '') {
  const match = String(subject).match(/\/\s*([0-9a-f]{7,40})\s*$/i);
...
export function getDeployPending() {
  ...
  const pendingBuild =
    ...
    (!dist.available || !builtFromSource || !hashesMatch(repo.shortHash, builtFromSource));
  ...
  if (pendingBuild) {
    message = `Há código novo no repo (${repo.shortHash}) que ainda não entrou em dist/. Rode Build + deploy completo.`;
    action = 'deploy:local';
