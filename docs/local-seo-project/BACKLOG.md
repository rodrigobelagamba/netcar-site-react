...
| TECH-001 | P0 | Todas | Confirmar 301/410 e metadados em produção | verificar produção | deploy atual | respostas e HTML registrados com data |
| ENTITY-001 | P0 | Esteio | Confirmar schema das duas lojas em produção | em andamento | publicar o build e verificar Google/site em produção | entidades válidas; dois perfis enviados; build validado e pronto em `dist/`, sem credenciais locais para upload |
...
| PERF-001 | P1 | Todas | Corrigir o primeiro lote de LCP/INP | pronto | medição por template | melhoria medida sem regressão funcional |
| SAP-001 | P1 | Sapucaia | Reforçar página local com utilidade e prova real | backlog | materiais locais | rota, bairros, estoque e prova autorizada |
| CAN-001 | P1 | Canoas | Reforçar página local com utilidade e prova real | em andamento | deploy, novo rastreamento e materiais locais | GSC auditado e texto factual corrigido no código em 22/08; produção e índice ainda pendentes |
